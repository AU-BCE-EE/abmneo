...
It is no longer a part of `var_pars`.
...
plot(slurry_mass ~ time, data = structure$dat, type = 'o')
```

Note: not `var_pars` needed.
...
Time-variable inputs are now separate from slurry mass series data, and can be combined with both of the above examples.
